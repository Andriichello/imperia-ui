<template>
  <div class="switcher" id="order-switcher">
    <div class="w-full flex justify-start items-center p-3 px-4 rounded selected gap-2 cursor-pointer"
            @click="onSwitchToDelivery(!showing)">

      <span class="loading loading-spinner loading-sm"
            v-if="isLoadingOrder || isCreatingOrder || isUpdatingOrder"
            :class="{'bg-black': theme === ThemeConfig.dark()}"/>

      <template v-if="isEmpty">
        <BaseIcon title="products" color="text-black" :width="32" :height="32" viewBox="0 0 64 64" v-if="!(isLoadingOrder)">
        ...
        <span class="w-full text-center text-black">
          {{ isLoadingOrder ? $t('preview.order.loading') : $t('preview.order.empty') }}
        </span>
      </template>
      ...
      <span class="loading loading-spinner loading-sm opacity-0"
            v-if="isLoadingOrder || isCreatingOrder || isUpdatingOrder"
            :class="{'bg-black': theme === ThemeConfig.dark()}"/>
    </div>
  </div>
...
export default defineComponent({
  name: 'DeliveryOrderSwitcher',
  components: {
    BaseIcon,
  },
  emits: ['switch-to-delivery'],
  props: {
    showArrow: {
      type: Boolean,
      default: true,
  ...
  data() {
    return {
      isNarrowScreen: window.innerWidth < 500,
    };
  },
  computed: {
    ThemeConfig() {
     ...
     ...mapGetters({
       theme: 'theme/get',
       showing: 'delivery/showing',
       productsTotal: 'delivery/productsTotal',
       productsCount: 'delivery/productsCount',
  ...
  methods: {
    priceFormatted,
    ...mapActions({
      setShowing: 'delivery/setShowing',
    }),
    onSwitchToDelivery(showing) {
      this.setShowing(showing);
      this.$emit('switch-to-delivery', showing);
    },
  },
})
</script>

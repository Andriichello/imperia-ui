  ...
  <nav>
    <div class="nav-block">
      <NavBarItem @item-click="onItemClick" page="home" path="/home" :selected="isSelected('home')" v-show="!onlyThemeSwitcher">
        <template v-if="restaurant && restaurant.media && restaurant.media.length">
          <img
              class="w-12 h-12 rounded"
              :alt="restaurant ? restaurant.place + ', ' + restaurant.city : 'home'"
              :title="restaurant ? restaurant.place + ', ' + restaurant.city : 'home'"
              :src="restaurant.media[0].url"
          />
        </template>

        <template v-else>
          <BaseIcon :title="restaurant ? restaurant.city : 'home'" :color="iconColor('home')" width="28" height="28" viewBox="0 0 24 24">

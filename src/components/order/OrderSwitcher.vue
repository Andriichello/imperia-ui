...
<!--        <span class="w-full text-black" v-if="allCount">-->
<!--          {{ allCount }} {{ allCount == 1 ? $t('preview.order.item_singular') : allCount > 1 && allCount < 5 ? $t('preview.order.item_plural2') : $t('preview.order.item_plural5') }} {{ $t('preview.order.for') }} {{ priceFormatted(allTotal) }}-->
<!--        </span>-->
        <span class="w-full text-black" v-if="productsCount">
          {{ productsCount }} {{ productsCount == 1 ? $t('preview.order.item_singular') : productsCount > 1 && productsCount < 5 ? $t('preview.order.item_plural2') : $t('preview.order.item_plural5') }} {{ $t('preview.order.for') }} {{ priceFormatted(productsTotal) }}

          <span class="w-full text-black" v-if="ticketsCount">
          + {{ ticketsCount }} {{ ticketsCount == 1 ? $t('preview.order.ticket_singular') : ticketsCount > 1 && ticketsCount < 5 ? $t('preview.order.ticket_plural2') : $t('preview.order.ticket_plural5') }} {{ $t('preview.order.for') }} {{ priceFormatted(ticketsTotal) }}
          </span>
        </span>
      ...
      <BaseIcon title="arrow" color="text-black" :width="24" :height="24" viewBox="0 0 24 24" style="transform: rotate(-90deg);" v-else-if="showArrow">

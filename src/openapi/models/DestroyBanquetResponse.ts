/* tslint:disable */
/* eslint-disable */
/**
 * imperia-api
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 0.1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
/**
 * Delete banquet response object.
 * @export
 * @interface DestroyBanquetResponse
 */
export interface DestroyBanquetResponse {
    /**
     * 
     * @type {string}
     * @memberof DestroyBanquetResponse
     */
    message: string;
}

/**
 * Check if a given object implements the DestroyBanquetResponse interface.
 */
export function instanceOfDestroyBanquetResponse(value: object): boolean {
    let isInstance = true;
    isInstance = isInstance && "message" in value;

    return isInstance;
}

export function DestroyBanquetResponseFromJSON(json: any): DestroyBanquetResponse {
    return DestroyBanquetResponseFromJSONTyped(json, false);
}

export function DestroyBanquetResponseFromJSONTyped(json: any, ignoreDiscriminator: boolean): DestroyBanquetResponse {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'message': json['message'],
    };
}

export function DestroyBanquetResponseToJSON(value?: DestroyBanquetResponse | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'message': value.message,
    };
}


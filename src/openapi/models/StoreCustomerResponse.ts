/* tslint:disable */
/* eslint-disable */
/**
 * imperia-api
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 0.1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
import type { Customer } from './Customer';
import {
    CustomerFromJSON,
    CustomerFromJSONTyped,
    CustomerToJSON,
} from './Customer';

/**
 * Store customer response object.
 * @export
 * @interface StoreCustomerResponse
 */
export interface StoreCustomerResponse {
    /**
     * 
     * @type {Customer}
     * @memberof StoreCustomerResponse
     */
    data: Customer;
    /**
     * 
     * @type {string}
     * @memberof StoreCustomerResponse
     */
    message: string;
}

/**
 * Check if a given object implements the StoreCustomerResponse interface.
 */
export function instanceOfStoreCustomerResponse(value: object): boolean {
    let isInstance = true;
    isInstance = isInstance && "data" in value;
    isInstance = isInstance && "message" in value;

    return isInstance;
}

export function StoreCustomerResponseFromJSON(json: any): StoreCustomerResponse {
    return StoreCustomerResponseFromJSONTyped(json, false);
}

export function StoreCustomerResponseFromJSONTyped(json: any, ignoreDiscriminator: boolean): StoreCustomerResponse {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'data': CustomerFromJSON(json['data']),
        'message': json['message'],
    };
}

export function StoreCustomerResponseToJSON(value?: StoreCustomerResponse | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'data': CustomerToJSON(value.data),
        'message': value.message,
    };
}

